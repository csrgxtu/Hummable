...
def receive_msg_handler(slack_manager, msg, sessions):
		...
		slack_group_name = msg.sender.name
		if isinstance(msg.sender, wxpy.MP):
				# open the mp group, send the msg here
				slack_group_name = '微信公众号'

		rtv = slack_manager.open_private_group(
				name=slack_group_name, wxid=msg.sender.wxid, sessions=sessions)
		if not rtv:
		...
		if slack_manager.send_msg_to_private_group(gid, text, as_user, user_name,
																							 icon_url):
